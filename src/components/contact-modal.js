...
export default function ContactModal({open, setOpen}) {
    ...
    function checkForErrors() {
        ...
        else if (!email.includes("@") || !email.includes(".")) tmp.push("valid_email");
        ...
        >
            <div className={"absolute inset-0 w-full h-full"}>
                ...
                >
                    ...
                    <div
                        className={"flex relative flex-col w-2/3 h-2/3 text-xl lg:text-2xl items-center justify-center space-y-6"}
                    >
                        <div className={`${sent ? "opacity-100" : "opacity-0"} transition-opacity pointer-events-none duration-500 bg-gray-300/80 absolute flex z-10 -inset-10 text-3xl font-bold justify-center items-center`}>
                            Your message was sent!
                        </div>
                        <div className={"flex w-full rounded-sm shadow-xl p-2 border border-black items-center justify-center"}>
                            <input
                                id={"inputName"}
                        ...
                        {showErrors && errors.length > 1 && (<div className={"flex w-full text-red-600 text-base items-center space-x-2 justify-center"}>
                            <FontAwesomeIcon icon={"exclamation-circle"} size={"sm"} className={"flex"}/>
                            <span className={"flex"}>missing required fields!</span>
                        </div>)}
                        {showErrors && errors.length === 1 && errors.includes("valid_email") && (<div className={"flex w-full text-amber-500 text-base items-center space-x-2 justify-center"}>
                            <FontAwesomeIcon icon={"exclamation-circle"} size={"sm"} className={"flex"}/>
                        ...
                        <div className={"flex w-full justify-end items-center align-end"}>
                            <button
                                ...
                                onClick={errors.length !== 0 ? () => setShowErrors(true) : () => handleContactSubmit()}
                            >
                                <span>Send</span>
                            </button>
                        </div>
